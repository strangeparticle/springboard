...
class FilterSpringboardForRuntimeTest {
    ...
    @Test
    fun `wasm hides terminal activators in filtered springboard`() {
        val full = SpringboardFactory.fromJson(TestFixtureJson.TERMINAL_ACTIVATOR, "/test/terminal.json")

        val filtered = filterSpringboardForRuntime(full, RuntimeEnvironment.WASM)

        val coordinate = Coordinate("dev", "app1", "res1")
        assertEquals(1, full.activators.filterIsInstance<TerminalActivator>().size)
        assertNotNull(full.indexes.activatorByCoordinate[coordinate])
        assertEquals(0, filtered.activators.filterIsInstance<TerminalActivator>().size)
        assertNull(filtered.indexes.activatorByCoordinate[coordinate])
    }

    @Test
    fun `desktop keeps terminal activators in filtered springboard`() {
        val full = SpringboardFactory.fromJson(TestFixtureJson.TERMINAL_ACTIVATOR, "/test/terminal.json")
        ...
        val coordinate = Coordinate("dev", "app1", "res1")
        assertNotNull(filtered.indexes.activatorByCoordinate[coordinate])
        assertEquals(1, filtered.activators.filterIsInstance<TerminalActivator>().size)
    }
