...
import com.strangeparticle.springboard.app.domain.mutator.updateEnvironment

internal class UpdateEnvironmentToolCallHandler : ToolCallHandler {
    override val providerToolId = "update_environment"
    override val description = "Update an existing environment."
    override val schema = requestSchema(UpdateEnvironmentToolCallHandlerRequest.serializer())

    suspend fun executeToolCallHandler(args: UpdateEnvironmentToolCallHandlerRequest, context: SpringboardToolCallExecutionContext): SpringboardToolCallHandlerResponse {
        ...
        return context.applyMutation(args.tab_id) { updateEnvironment(springboard, args.id, args.name) }
    }
    ...
    override suspend fun executeToolCallHandler(toolCallId: String, argumentsAsJsonString: String, context: ToolCallExecutionContext): ToolCallHandlerResponse {
        val args = decodeToolCallHandlerRequest(argumentsAsJsonString, UpdateEnvironmentToolCallHandlerRequest.serializer())
